                ...
                if (!response.url().includes('web.archive.org')){
                  ...
                  Object.keys(headers).forEach((key) => {
                    if (key.match(/x-archive-orig/gi)){
                      ...
                      this.headers[waybackKey] = Array.isArray(headers[key]) ? headers[key] : [headers[key]];
                      if (waybackKey == 'set-cookie'){ // Mannually set the cookie for x-archive-orig-set-cookie
                        const cookie_str = this.headers[waybackKey];
                        Object.keys(cookie_str).forEach(async (key)=>{
                          const v = cookie_str[key];
