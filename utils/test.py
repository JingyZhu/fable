...
sys.path.append('../')
import config
from utils import sic_transit, broken

broken_testcase =     testcase = [
...
def test_broken(bc):
    """
    Test broken classification for combine techniques of content comparison and optimized sic transit
    """
    if bc is None: bc = broken.BrokenClassifier()
    print('url\tlabel\tclassification')
    for tc in testcase:
        url, label = tc
        clas = bc.broken(url)[0]
        print(f'{url}\t{label}\t{clas}')